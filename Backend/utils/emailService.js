...
export const senderVerficationEmail = async (email, name, otp) => {
  return sendEmail({
    to: [{ email, name }],
    subject: "Your Verfication Code - Job Portal",
    htmlContent: otpTemplate(
      "Verify YOur email",
      name,
      otp,
      "Thank you for signing up.please verify 6 -digit code to verify your email address",
    ),
  });
};

// to send Forget Password
export const sendForgotPassword = async (email, name, otp) => {
  return sendEmail({
    ...
    htmlContent: otpTemplate(
      ...
      name,
      otp,
      "YOur response to reset your password.Please use the following 6-digit code to process",
    ),
  });
};

// send to admin inquery
